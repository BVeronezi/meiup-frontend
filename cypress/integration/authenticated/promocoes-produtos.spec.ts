...
describe("Promoção Produto", () => {
  ...
  it("Realiza corretamente o cadastro do promoção produto", () => {
    ...
    cy.get(`[data-cy="produtos"]`).should("be.visible").click();
    ...
    cy.get('[data-cy="adicionar-produto"]').should("be.visible");
    cy.get('[data-cy="adicionar-produto"]').click();

    cy.get("#table-produtos-promocao")
      .should("be.visible")
      .should("be.visible")
      .contains("td", "Couve");
